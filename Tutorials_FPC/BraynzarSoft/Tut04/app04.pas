...
// -----------------------------------------------------------------------------
//
// separate file, extracetd from Tut04.lpr - 2021-04-23
//
//
//  HINT:  does not work well, EXCEPTION occurs in  RtlCaptureStackContext:
//
//  77CD72AB eb0a          jmp    0x77cd72b7 <ntdll!RtlCaptureStackContext+3111>
//
// -----------------------------------------------------------------------------
...
const
    ...
    WindowName: String = 'BzTutsApp';

    // title of the window
    WindowTitle: String = 'Bz Window';
...
begin
    ...
    wc.lpszMenuName := nil;
    wc.lpszClassName := PAnsiChar( WindowName);
    ...
    aHandle:= CreateWindowExA( 0,
                               PAnsiChar(WindowName),
...
var
    ...
    //lUseWarpDevice: boolean = False;     not in use
    lUseDebug: boolean = False;
    //debugdevice: ID3D12DebugDevice;      not in use

    dxgiFactoryFlags: UINT = 0;
    //iTemp: UINT;                         not in use
...
begin
    ...
    swapChainDesc.OutputWindow := aHandle; // handle to our window
    ...
    if not FileExists( 'x86_64-win64/VertexShader.hlsl')
      then Exit;

    hr := D3DCompileFromFile( 'x86_64-win64/VertexShader.hlsl',
                              nil, nil, 'main', 'vs_5_0',
                              D3DCOMPILE_DEBUG or D3DCOMPILE_SKIP_OPTIMIZATION,
    ...
    if not FileExists( 'x86_64-win64/PixelShader.hlsl')
      then Exit;

    hr := D3DCompileFromFile( 'x86_64-win64/PixelShader.hlsl', nil, nil, 'main', 'ps_5_0', D3DCOMPILE_DEBUG or D3DCOMPILE_SKIP_OPTIMIZATION,
